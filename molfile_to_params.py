...
def build_fragment_trees(molfile): #{{{
    ...
    for frag_id in set([a.fragment_id for a in molfile.atoms]):
        ...
        def tree_dfs(parent):
            ...
            keyfunc=functools.cmp_to_key(lambda a,b: cmp(a.is_H, b.is_H))
            tmp_children.sort(key=keyfunc)
    ...
    if not options.no_param:
        for i in range(num_frags):
            ...
            if not options.clobber and os.path.exists(param_file):
                ...
                write_param_file(param_file, m, options.name, i+1, len(molfiles), options.max_confs, options.amino_acid)
